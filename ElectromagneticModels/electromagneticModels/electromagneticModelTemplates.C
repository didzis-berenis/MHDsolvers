...
namespace Foam
{
    ...
    )
    {
        ...
        if (electroDict.isDict("electromagneticType"))
        {
            const dictionary& electroTypeDict(electroDict.subDict("electromagneticType"));
            const word modelType(electroTypeDict.name());

            Info<< "Selecting thermodynamics package " << modelType << endl;

            if (electroTypeDict.found("complex"))
            {
                const bool isComplex(electroTypeDict::lookupOrDefault<bool>("complex",false));
            }
        }
        else
        {
            const word modelType(electroDict.lookup("electromagneticType"));
            // Default is false
